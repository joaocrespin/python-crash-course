...
class RainGame():
    ...
    def _update_rain_drops(self):
        ...
        self._check_rain_drop_edges()
        self.rain_drops.update()

    def _check_rain_drop_edges(self):
        '''Check whether the rain drops are at edge.'''
        for drop in self.rain_drops.sprites():
            if drop.check_edges():
                self._constant_rain_drops()
                break

    def _constant_rain_drops(self):
        '''Delete the entire row and create new rain drops.'''
        for drop in self.rain_drops.copy():
